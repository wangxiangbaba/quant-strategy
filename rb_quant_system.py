...
def fetch_rebar_data(symbol: str = "RB0") -> pd.DataFrame:
    """
    从 AkShare 获取螺纹钢主力连续合约日线数据。

    说明：
    - 新版本 AkShare 中已不再提供 futures_main_ctp，
      使用官方当前推荐的 `futures_main_sina` 接口替代。
    - symbol 示例：
      * 螺纹钢主力: "RB0"
    """
    log.info(f"正在获取螺纹钢数据: {symbol} ...")
    # AkShare 文档推荐使用 futures_main_sina 获取主力连续数据
    ...
    log.info(f"数据获取完成: {len(df)} 条记录，时间范围 {df.index[0].date()} ~ {df.index[-1].date()}")
    return df
...
def add_indicators(df: pd.DataFrame, short_p: int, long_p: int,
                   rsi_p: int = 14, atr_p: int = 14) -> pd.DataFrame:
...
class BacktestConfig:
    ...
    # RSI 过滤
    rsi_p:    int   = 14
    rsi_ob:   float = 70.0   # 超买（多头不追）
    rsi_os:   float = 30.0   # 超卖（空头不追）
    # ATR 波动率过滤
    atr_p:    int   = 14
    atr_mul:  float = 0.8    # TR > ATR * atr_mul 才入场
    # 止损
    stop_atr_mul: float = 2.0  # 止损距离 = ATR * stop_atr_mul
    # 成本
    commission: float = 0.0001  # 万分之一手续费
    slippage:   int   = 1       # 滑点 Tick 数
    tick_size:  float = 1.0     # 螺纹钢最小变动 1元/吨
...
def run_backtest(df_raw: pd.DataFrame, cfg: BacktestConfig) -> pd.DataFrame:
    ...
    # ── 收益计算（含手续费与滑点） ────────────────
    cost_per_trade = (
        df["close"] * cfg.commission
        + cfg.slippage * cfg.tick_size
    ) / df["close"]   # 转化为百分比影响
...
def calc_max_drawdown(equity: pd.Series) -> tuple[float, int]:
    ...
    dd_lengths = underwater * (underwater.groupby(
        (underwater == 0).cumsum()).cumcount() + 1)
...
def calc_consecutive(series: pd.Series) -> tuple[int, int]:
    ...
    def max_run(s):
        cumsum = s.cumsum()
        return (cumsum - cumsum.where(s == 0).ffill().fillna(0)).max()

    return int(max_run(wins)), int(max_run(losses))
...
def param_plain_scan(df_raw: pd.DataFrame,
                     short_range=(5, 25, 5),
                     long_range=(20, 80, 10)) -> pd.DataFrame:
    """
    扫描均线参数组合，输出夏普比率热力图。
    用夏普比率（而非总收益）衡量更客观。
    """
...
def plot_param_plain(results: pd.DataFrame, save_path: str | None = None):
    ...
    plt.title("螺纹钢 — 双均线参数平原（夏普比率）", fontsize=14, fontweight="bold")
    plt.xlabel("长周期均线", fontsize=12)
    plt.ylabel("短周期均线", fontsize=12)
    plt.tight_layout()
    if save_path is None:
        save_path = "param_plain.png"
...
def plot_strategy(df: pd.DataFrame, symbol: str = "RB", save_path: str | None = None):
    ...
    ax1.plot(df.index, df["sma_s"], color="#1e90ff", lw=1.5, label=f"短均线")
    ax1.plot(df.index, df["sma_l"], color="#ff4500", lw=1.5, label=f"长均线")

    buy_idx  = df.index[df["signal"].diff() > 0]
    sell_idx = df.index[df["signal"].diff() < 0]
    ax1.scatter(buy_idx,  df.loc[buy_idx,  "close"], marker="^", color="lime",
                s=80, zorder=5, label="开多")
    ax1.scatter(sell_idx, df.loc[sell_idx, "close"], marker="v", color="red",
                s=80, zorder=5, label="平/开空")
    ax1.set_title(f"{symbol} 螺纹钢量化策略", fontsize=13, fontweight="bold")
    ax1.legend(loc="upper left", fontsize=9)
    ax1.grid(alpha=0.3)

    # ── 子图2：权益曲线对比 ──
    ax2.plot(df.index, df["equity"],     color="#2ecc71", lw=2, label="净收益（扣费后）")
    ax2.plot(df.index, df["equity_raw"], color="#95a5a6", lw=1, linestyle="--", label="毛收益")
...
def calc_position_size(total_equity: float, risk_ratio: float,
                       current_atr: float, multiplier: int = 10,
                       stop_atr_mul: float = 2.0) -> int:
    """
    ATR动态仓位计算：
      手数 = (账户总权益 × 单笔风险比例) / (ATR × 止损倍数 × 合约乘数)
    建议：wind市场中 stop_atr_mul 取 2，震荡市取 1.5
    """
    stop_distance = current_atr * stop_atr_mul   # 元/吨
    loss_per_lot  = stop_distance * multiplier    # 元/手
    if loss_per_lot <= 0:
        return 1
...
# ╚══════════════════════════════════════════════╝
# 注意：运行实盘部分需要安装 tqsdk 并拥有天勤账户
# pip install tqsdk

LIVE_CONFIG = {
    "symbol":           "SHFE.rb2605",  # ⚠ 每季度确认主力合约
    ...
    "stop_atr_mul":     2.0,
    "risk_per_trade":   0.02,
    "max_loss_limit":   5000,            # 熔断线（元）
    "phone":            "你的手机号",
    "password":         "你的密码",
}

TRADING_WINDOWS = [
    (time(9, 1),  time(10, 14)),   # 上午第一节
    (time(10, 31), time(11, 29)),  # 上午第二节
    (time(13, 31), time(14, 55)),  # 下午（14:55后不新开仓）
...
def check_contract_expiry(symbol: str) -> None:
    ...
    if month in (1, 5, 9):
        log.warning(f"⚠ 当前月份 {month} 月，螺纹钢主力可能切换，请确认合约: {symbol}")
...
def run_live():
    ...
    try:
        ...
        log.info(f"✅ 实盘系统启动 | 合约:{cfg['symbol']} | 初始资金:¥{init_balance:,.0f}")
        log.info(f"   当前持仓: 多{position.pos_long}手 / 空{position.pos_short}手")
        ...
        while True:
            ...
            if equity < (init_balance - cfg["max_loss_limit"]):
                if not is_melted:
                    log.error(f"🔴 熔断触发！当前权益 ¥{equity:,.0f}，强制全平！")
            ...
            # ── C. K线更新后计算指标 ─────────────────
            if not (api.is_changing(klines.iloc[-1], "datetime") and len(klines) > cfg["long_p"] + 5):
                continue
            ...
            lots = calc_position_size(
                equity, cfg["risk_per_trade"], atr,
                stop_atr_mul=cfg["stop_atr_mul"]
            )

            if ma_s > ma_l and rsi < cfg["rsi_ob"] and cur_pos <= 0:
                log.info(f"📈 多头信号 | 价格:{close} MA短:{ma_s:.1f} MA长:{ma_l:.1f} RSI:{rsi:.1f} | 建议{lots}手")
                tpos.set_target_pos(lots)
                high_entry = close

            elif ma_s < ma_l and rsi > cfg["rsi_os"] and cur_pos >= 0:
                log.info(f"📉 空头信号 | 价格:{close} MA短:{ma_s:.1f} MA长:{ma_l:.1f} RSI:{rsi:.1f} | 建议{lots}手")
                tpos.set_target_pos(-lots)
                low_entry = close
            ...
            if cur_pos > 0:
                ...
                if close < high_entry - stop_dist:
                    log.warning(f"⛔ 多头ATR止损 | 最高:{high_entry:.1f} 当前:{close:.1f} 止损距:{stop_dist:.1f}")
                    tpos.set_target_pos(0)
                ...
                if close > low_entry + stop_dist:
                    log.warning(f"⛔ 空头ATR止损 | 最低:{low_entry:.1f} 当前:{close:.1f} 止损距:{stop_dist:.1f}")
                    tpos.set_target_pos(0)
...
if __name__ == "__main__":
    ...
    log_file_path = os.path.join(log_run_dir, "rb_strategy.log")
    ...
    if mode == "live":
        ...
        # ── 参数平原扫描 ──
        df_raw = fetch_rebar_data("RB0")
        results = param_plain_scan(df_raw)

        # 为本次扫描创建独立结果目录: results/RB/RB_时间戳
        ...
        heatmap_path = os.path.join(run_dir, "rb_param_plain.png")
        ...
        # 为本次回测创建独立结果目录: results/RB/RB_时间戳
        base_results_dir = os.path.join(base_dir, "results", product_code)
        os.makedirs(base_results_dir, exist_ok=True)
        run_dir = os.path.join(base_results_dir, f"{product_code}_{ts}")
        os.makedirs(run_dir, exist_ok=True)
        ...
        print_report(report)
        report_path = os.path.join(run_dir, "rb_backtest_report.txt")
        chart_path = os.path.join(run_dir, "rb_strategy_chart.png")
        save_report_txt(report, report_path)
        plot_strategy(df_bt, save_path=chart_path)

        log.info("回测完成。运行方式:")
        log.info("  python rb_quant_system.py            # 回测")
        log.info("  python rb_quant_system.py scan       # 参数平原扫描")
        log.info("  python rb_quant_system.py live       # 实盘/模拟盘")
